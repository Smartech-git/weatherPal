    ...
    <div className='searchedCityCard'>
      ...
      </div>
      {
        (props.error || props.data === undefined)? (
          <div className='searchedCityCard' style={{display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: state.themeHue.primary_light}}>{props.data === undefined ? <h1 style ={{ color: state.themeHue.base }} className='viewMoreLoading'>Loading . . .</h1> : <h1 style ={{ color: state.themeHue.base }} className='viewMoreLoading'>Can't fetch data at the moment &#129402;</h1>}</div>
        ) : (
